...
export default function GroupRatioSettings(props) {
  ...
  const renderVisualMode = () => (
    <Form key='form-visual' values={inputs} className='na-settings-form'>
      <Form.Section text={t('分组管理')}>
        <Text type='tertiary' size='small' className='na-settings-note'>
          {t(
          ...
          <Col xs={24} sm={12} md={8} lg={8} xl={8}>
            <Form.Slot label={t('默认使用auto分组')}>
              ...
              <Text
                type='tertiary'
                size='small'
                className='na-settings-help-copy'
              >
  ...
  const GuideSection = ({ title, children }) => {
    ...
    return (
      <div className='na-settings-guide-block'>
        <Button
          theme='borderless'
          size='small'
          icon={open ? <IconChevronUp /> : <IconChevronDown />}
          onClick={() => setOpen(!open)}
          className='na-settings-guide-toggle'
        >
          {title}
        </Button>
        <Collapsible isOpen={open} keepDOM>
          <div className='na-settings-guide-content'>{children}</div>
        </Collapsible>
      </div>
    );
  };
  ...
  const CodeBlock = ({ children }) => (
    <pre className='na-settings-guide-code'>{children}</pre>
  );
  ...
  const renderGuide = () => (
    <SideSheet
      ...
      bodyStyle={{
        overflow: 'auto',
        padding: '0 var(--na-space-6) var(--na-space-6)',
      }}
    >
      <Tabs type='line' size='small'>
        <Tabs.TabPane tab={t('概览')} itemKey='overview'>
          <div className='na-settings-section-spacer'>
            <Title heading={5}>{t('什么是分组？')}</Title>
            <Paragraph className='na-settings-help-copy'>
              {t(
        ...
        <Tabs.TabPane tab={t('分组管理')} itemKey='groups'>
          <div className='na-settings-section-spacer'>
            ...
            <GuideSection title={t('查看示例')}>
              ...
              <Paragraph size='small' className='na-settings-help-copy'>
                {t(
            ...
            <GuideSection title={t('JSON 格式参考')}>
              <Paragraph size='small' className='na-settings-note'>
                <Text strong code>
                  GroupRatio
                </Text>
                {' — '}
